...
`dsync` 是 docker-image-sync-platform 的 CLI 客户端，通过平台 REST API（`/api/v1`）完成登录、镜像仓库（阿里云 ACR / 华为云 SWR / 腾讯云 CCR / Harbor / 通用 Registry）/镜像/Tag 查询、镜像同步提交与进度跟踪，适合日常使用与脚本自动化。

> 平台支持多类型仓库实例混管（ACR/SWR/CCR/Harbor/通用 Registry），统一以**别名（ALIAS）**标识展示与选择——各厂商命名空间可能同名，需用别名区分；`--acr` 参数优先按别名匹配，也兼容直接传 namespace（同名冲突时会要求改用别名）。SWR 的 namespace 即组织名。
...
| `dsync check <镜像>... [--acr ns]` | 检查镜像与仓库归属冲突 |
| `dsync suggest <镜像>` | 推荐目标仓库及理由 |
...
- `--acr <alias>`：指定目标镜像仓库（别名优先，兼容 namespace）。**不指定时按服务端亲和性逻辑自动选择**（与 Web 端一致），CLI 会打印所选仓库与理由。
...
1. **仓库归属**：仓库已归属其他仓库时拦截（避免跨仓库重复建仓）；
2. **Tag 级查重**：目标仓库已存在相同 `仓库:Tag` 时拦截（纯重复同步）。
...
**Q: 提示"未找到别名为 xxx 的镜像仓库"或"xxx 同时是多个仓库的 namespace"？**
