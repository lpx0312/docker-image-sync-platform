...
| T8 | GitHub Actions | 检查 API 限制、状态筛选、运行详情弹层、用量卡片与仓库排行折叠 |
...
# 预期（仓库数 > 10 时）：显示全部仓库，按钮变为「收起其余仓库」；再次点击恢复前 10
```

**通过标准**：限制数据有值，筛选生效，详情弹层字段完整；用量四格有数据且排行面板收起/展开/二级展开均正常。
